...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Cek apakah ada perubahan room_id
    if ($room_id != $booking['room_id']) {
        ...
        // Jika tidak ada perubahan room_id, hanya update data booking
        $update_sql = "UPDATE bookings SET check_in = ?, check_out = ?, adults = ?, children = ?, special_request = ? WHERE booking_id = ?";
        $update_stmt = $conn->prepare($update_sql);
        $update_stmt->bind_param("ssiisi", $checkin, $checkout, $adults, $children, $special_request, $booking_id);

        if ($update_stmt->execute()) {
            // Redirect kembali ke halaman detail booking setelah update
